...
test('handleTextRequest: title + body -> 200 with slug and minutes', () => {
  const result = handleTextRequest({ title: 'Hello World', body: 'one two three' });
  assert.deepStrictEqual(result, { status: 200, body: { slug: 'hello-world', minutes: 1 } });
});
...
test('handleTextRequest: rounds reading time up to whole minutes', () => {
  ...
  assert.deepStrictEqual(result, { status: 200, body: { slug: 'long-read', minutes: 2 } });
});
