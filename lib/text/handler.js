const { slugify, readingTime } = require('./index');

// Query params can arrive as arrays (repeated keys) or objects, so coerce to a
// plain string before handing anything to slugify/readingTime.
...
function handleTextRequest(query) {
  ...
  return { status: 200, body: { slug: slugify(title), minutes: readingTime(body) } };
}
